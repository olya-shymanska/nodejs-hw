...
export const getNoteById = async (req, res) => {
  ...
  const noteById = await Note.findOne({ _id: noteId, userId: req.user._id });
...
export const updateNote = async (req, res) => {
  const { noteId } = req.params;
  const body = req.body;
  const note = await Note.findOneAndUpdate({ _id: noteId, userId: req.user._id }, body, {returnDocument: 'after',});
